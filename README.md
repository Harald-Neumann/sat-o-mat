...
There is a `variables`, a `steps` block and a `cleanup` block.
...
The `start` variable is optional, meaning the schedule entry should be executed immediately.
The `end` variable is mandatory and may be relative to `$start`.
...
The `cleanup` block is like `steps`, but always gets executed at the end of a schedule entry.

### Artifacts

The commands in `steps` and `cleanup` are executed with the current working directory (CWD) set to a new directory that can be used to store artifacts generated by the schedule entry's execution.
